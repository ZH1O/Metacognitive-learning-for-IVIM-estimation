...
for i in range(3):
    plt.subplot(1, 3, i + 1)
    plt.xticks([0.02, 0.04, 0.06, 0.08, 0.1], fontsize=80)
    plt.yticks(ysticks[i], fontsize=80)
    ...
    plt.ylabel(plotname[i], fontsize=100)
    plt.ylim(ylim_list[i][0], ylim_list[i][1])
    plt.xlabel("noise level", fontsize=100)
    plt.legend().set_visible(False)
plt.subplots_adjust(wspace=0.4, hspace=0.)
handles, labels = plt.gca().get_legend_handles_labels()
fig = plt.gcf()
fig.legend(handles, labels, loc='lower center', ncol=5, fontsize=80, columnspacing=1.0, handletextpad=1,bbox_to_anchor=(0.5, -0.16), bbox_transform=plt.gcf().transFigure)
...
for i in range(3):
    ...
    plt.xticks([0.02, 0.04, 0.06, 0.08, 0.1], fontsize=80)
    plt.yticks(ysticks[i], fontsize=80)
    sns.lineplot(x="SNR", y=ivimname[i], hue="Strategy", data=df, linewidth=15, palette=palette, marker='h', markersize=30,markeredgecolor=None)
    plt.ylabel(plotname[i], fontsize=100)
    plt.ylim(ylim_list[i][0], ylim_list[i][1])
    plt.xlabel("noise level", fontsize=100)
    plt.legend().set_visible(False)
...
handles, labels = plt.gca().get_legend_handles_labels()
fig = plt.gcf()
fig.legend(handles, labels, loc='lower center', ncol=5, fontsize=180, columnspacing=1.0, handletextpad=1,bbox_to_anchor=(0.5, -0.16), bbox_transform=plt.gcf().transFigure)
